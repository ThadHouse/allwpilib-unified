...
namespace sim {
  ...
  typedef int32_t (*RegisterChannelCallbackFunc)(int32_t index, int32_t channel, HAL_NotifyCallback callback, void* param, HAL_Bool initialNotify);
  typedef void (*FreeChannelCallbackFunc)(int32_t index, int32_t channel, int32_t uid);
  ...
  SIM_JniHandle AllocateChannelCallback(JNIEnv* env, jint index, jint channel, jobject callback, jboolean initialNotify, RegisterChannelCallbackFunc createCallback);
  ...
  void FreeChannelCallback(JNIEnv* env, SIM_JniHandle handle, jint index, jint channel, FreeChannelCallbackFunc freeCallback);
